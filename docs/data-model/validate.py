...
declared entity and an existing column. Also reports entity/edge counts.
    ...
    python3 docs/data-model/validate.py     # exit 0 = all endpoints resolve

Written 2026-08-14, extended and moved into the repo 2026-08-15. It catches a moved or
renamed column that a relationship line still points at, AND it now checks
that the files agree with each other — the two structural files are the same
schema, the full public view matches through the rename map, and every scoped
view is a genuine subset. That last part used to be "still a read".

The source of truth is docs/data-model/MODEL.md; schema.eraser is its
structural expression, and this script is what keeps the diagrams honest
against it.
...
D = REPO / "docs/data-model"
FILES = [
    D / "schema.eraser",    # THE SCHEMA OF RECORD — implement from this one
    D / "full.eraser",      # the same model in buyer words, one canvas
    D / "catalog.eraser",   # "what is this pen, and what does it come as?"
    D / "fit.eraser",       # "will this refill fit?"        <- review this first
    D / "remedies.eraser",  # "it doesn't drop in. now what?"
    D / "collection.eraser",# "what do I own, and what's loaded?"
]
...
for path in FILES:
    name = path.name
    if name in ("schema.eraser", "full.eraser"):
        continue
