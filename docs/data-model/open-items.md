...
**Where the truth lives.** `docs/archive/data-model-corpus/data-model-answers.md` is the source of truth and every row
...
| H3 | **eraser.io automation.** The `ERASER DOC URL: ____` slots are **still blank** — nothing has ever been pushed; every render is a manual paste. **`docs/data-model/README.md` now has a URL table to fill in as you render**, which closes the *referencing* half without the automation. | Mechanics are settled and verified in `docs/archive/data-model-corpus/eraser-integration-findings.md`; wiring it was never started |
| H4 | ✅ **RESOLVED 2026-08-15 by the distillation commit.** The whole model now ships in git: `docs/data-model/` holds `MODEL.md` (the source of truth), all five diagrams and `schema.eraser` — which was `data-model-erd-clean.eraser`, the file implementation is built from — and the working corpus is archived verbatim at `docs/archive/data-model-corpus/`. Nothing in `.notes/` is load-bearing any more. | Closed. Was: "the rest of the corpus is still scratch-only; worth deciding whether it belongs in `docs/` too" |
...
`python3 docs/data-model/validate.py` → **exit 0**. It prints the counts for all six ERD sources,
