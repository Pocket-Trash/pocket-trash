...
describe("scraper env", () => {
  ...
  it("defaults scheduler settings for scraper jobs", () => {
    ...
    expect(env.SCRAPER_GRIMSMO_FJELL_START_DELAY_SECONDS).toBe(0);
    expect(env.SCRAPER_GRIMSMO_INTERVAL_MINUTES).toBe(60);
    expect(env.SCRAPER_GRIMSMO_NORSEMAN_START_DELAY_SECONDS).toBe(0);
    expect(env.SCRAPER_GRIMSMO_RASK_START_DELAY_SECONDS).toBe(0);
